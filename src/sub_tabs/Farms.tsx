...
import { FARM_COST, FOOD_PER_GATHER } from "../lib/constants";

function gatherFood() {
    useGameStore.setState(s => ({ food: s.food + FOOD_PER_GATHER }));
}

function makeFarm() {
    useGameStore.setState(s => {
        if (s.wood >= FARM_COST.wood && s.stone >= FARM_COST.stone) {
            return { farms: s.farms + 1, wood: s.wood - FARM_COST.wood, stone: s.stone - FARM_COST.stone };
        }
        return s;
    });
}
                ...
                <div className="w-full h-full flex flex-col gap-5">
                    ...
                    <div className="agriculture_unlocked relative inline-block group">
                        ...
                        <div className="absolute top-full z-10 left-1/2 mt-2
                                        ...
                                        transition-opacity duration-200
                                        p-2 text-sm max-w-150 min-w-50 min-h-0 border-2 bg-main2">
                            costs {FARM_COST.wood} wood and {FARM_COST.stone} stone.
                        </div>
                    </div>
                    <hr ref={progressRef} className="agriculture_unlocked border-3 border-emerald-600 w-0 rounded-full" />
                </div>
